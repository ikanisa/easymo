...
import { createClient } from "https://esm.sh/@supabase/supabase-js.76.1";
...
const WINDOW_MINUTES = Number(
  Deno.env.get("RECURRING_TRIPS_WINDOW_MINUTES") ?? "5",
);
const CRON_EXPR = "*/1 * * * *";
...
const CRON_ENABLED =
  (Deno.env.get("RECURRING_TRIPS_CRON_ENABLED") ?? "true").toLowerCase() !==
    "false";
...
async function shortlistTrip(trip: any) {
  ...
  if (favoriteError) {
    console.error(
      "recurring-trips-scheduler.favorite_lookup_failed",
      favoriteError,
    );
    return null;
  }
  ...
  if (!origin || !destination) {
    console.warn("recurring-trips-scheduler.favorite_missing", {
      trip_id: trip.id,
    });
    return null;
  }
  ...
  const { data: parkingData, error: parkingError } = await supabase.rpc(
    "search_driver_parking_candidates",
    {
      _pickup_lat: originCoords.lat,
      _pickup_lng: originCoords.lng,
      _dropoff_lat: destinationCoords.lat,
...
async function runScheduler(trigger: "http" | "cron") {
  ...
  const results = [] as Array<
    { trip_id: string; live_candidates: number; parking_candidates: number }
  >;
  ...
  console.info("telemetry.broker_shortlist_generated", {
    ...
    count: results.reduce(
      (acc, item) => acc + item.parking_candidates + item.live_candidates,
      0,
    ),
  });
