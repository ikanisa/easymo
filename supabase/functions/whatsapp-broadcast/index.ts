 ...
 * - Supports candidate_vendors (new flow) and vendors table (legacy)
...
interface BroadcastRequest {
  ...
  candidateIds?: string[]; // New: candidate vendor IDs from candidate_vendors table
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { requestId, userPhone, userLocationLabel, needDescription, vendorFilter, candidateIds } = body;
    ...
    await logStructuredEvent("BROADCAST_STARTED", {
      ...
      hasCandidateIds: !!candidateIds && candidateIds.length > 0
    });
    ...
    const { data: broadcastRecord, error: broadcastError } = await supabase
      ...
      .insert({
        ...
        user_location_label: userLocationLabel || null,
    ...
    // Get target vendors - prioritize candidate_vendors if candidateIds provided
    ...
    if (candidateIds && candidateIds.length > 0) {
      // NEW FLOW: Use candidate_vendors from the AI search
      const { data: candidates, error: candidatesError } = await supabase
        .from("candidate_vendors")
        .select("id, name, phone, source, is_onboarded")
        .in("id", candidateIds)
        .not("phone", "is", null); // Only businesses with phone numbers

      if (candidatesError) {
        throw new Error(`Failed to query candidate vendors: ${candidatesError.message}`);
      }

      if (!candidates || candidates.length === 0) {
        await logStructuredEvent("BROADCAST_NO_CANDIDATES", {
          requestId,
          candidateIds,
          correlationId
        }, "warn");
        ...
        return new Response(
          JSON.stringify({
            ...
            sentCount: 0,
            errorCount: 0,
            message: "No eligible candidate vendors found"
          }),
          { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }

      // Map candidates to vendor format
      eligibleVendors = candidates.map((c: any) => ({
        id: c.id,
      ...
      await logStructuredEvent("BROADCAST_USING_CANDIDATES", {
        requestId,
        candidateCount: eligibleVendors.length,
        correlationId
      });

    } else {
      // OLD FLOW: Query vendors table (for backwards compatibility)
