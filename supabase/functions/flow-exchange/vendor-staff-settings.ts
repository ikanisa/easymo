...
export async function handleStaffList(payload: Record<string, unknown>, supabase: SupabaseClient) {
  ...
  const staff_text = (data ?? [])
    .map((entry) => `${entry.role}: ${entry.number_e164}${entry.is_active ? '' : ' (inactive)'}`)
    .join('\n');
...
export async function handleStaffAdd(payload: Record<string, unknown>, supabase: SupabaseClient) {
...
export async function handleStaffRemove(payload: Record<string, unknown>, supabase: SupabaseClient) {
  const parsed = staffActionSchema.parse({
    bar_id: resolveField(payload, 'bar_id'),
    phone: resolveField(payload, 'phone'),
  });
...
export async function handleSaveSettings(payload: Record<string, unknown>, supabase: SupabaseClient) {
