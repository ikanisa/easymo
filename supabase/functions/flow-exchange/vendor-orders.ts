...
const queueSchema = z.object({
  ...
  queue: z.enum(['pending', 'paid', 'served', 'cancelled']).optional().default('pending'),
});
...
export async function handleVendorQueue(payload: Record<string, unknown>, supabase: SupabaseClient) {
  ...
  const items = (data ?? []).map((order) => ({
    ...
    description: `${new Date(order.created_at).toLocaleTimeString()} · ${formatCurrency(order.total_minor ?? 0)}`,
  }));
...
export async function handleVendorOrderDetail(payload: Record<string, unknown>, supabase: SupabaseClient) {
  ...
  const { data: order, error } = await supabase
    .from('orders')
    .select('id, order_code, table_label, total_minor, subtotal_minor, service_charge_minor, status, created_at, note')
  ...
  const lines = (items ?? []).map((item) => `${item.qty}x ${item.item_name} — ${formatCurrency(item.line_total_minor ?? 0)}`).join('\n');
...
export async function handleVendorMarkPaid(payload: Record<string, unknown>, supabase: SupabaseClient) {
  const parsed = orderActionSchema.parse({
    bar_id: resolveField(payload, 'bar_id'),
    order_id: resolveField(payload, 'order_id'),
  });
...
export async function handleVendorMarkServed(payload: Record<string, unknown>, supabase: SupabaseClient) {
  const parsed = orderActionSchema.parse({
    bar_id: resolveField(payload, 'bar_id'),
    order_id: resolveField(payload, 'order_id'),
  });
...
export async function handleVendorCancel(payload: Record<string, unknown>, supabase: SupabaseClient) {
  const parsed = orderActionSchema.parse({
    bar_id: resolveField(payload, 'bar_id'),
    order_id: resolveField(payload, 'order_id'),
...
async function fetchOrderNotificationInfo(supabase: SupabaseClient, orderId: string) {
  const { data, error } = await supabase
    .from('orders')
    .select(
      'id, order_code, table_label, bar_id, customers ( wa_id ), bars ( name )'
    )
    .eq('id', orderId)
    .maybeSingle();
  if (error) {
  ...
  return {
    ...
    customerWaId: data.customers?.wa_id ?? null,
  };
}
