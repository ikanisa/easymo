...
export async function handleCategoryMove(
  payload: Payload,
  supabase: SupabaseClient,
) {
...
export async function handleCategoryRename(
  payload: Payload,
  supabase: SupabaseClient,
) {
  const parsed = categoryActionSchema.parse({
...
export async function handleCategoryAdd(
  payload: Payload,
  supabase: SupabaseClient,
) {
  const parsed = categoryActionSchema.parse({
...
export async function handleItemSave(
  payload: Payload,
  supabase: SupabaseClient,
) {
  const parsed = itemActionSchema.parse({
...
export async function handleBulkPrices(
  payload: Payload,
  supabase: SupabaseClient,
) {
  const parsed = itemActionSchema.parse({
  ...
  if (
    !parsed.mode || !parsed.amount_type || parsed.amount_value === undefined
  ) {
    return buildErrorResponse(
      "s_items_list",
      "Provide mode, amount type, and value.",
    );
  }
  ...
  for (const update of updates) {
    await supabase.from("items").update({ price_minor: update.price_minor }).eq(
      "id",
      update.id,
    );
  }
