...
export class VendorResponseHandler {
  ...
  async formatUserNotification(sessionId: string): Promise<string | null> {
    try {
      ...
      const vendorResponses = await outreachService.getVendorResponses(
        sessionId,
      );
      ...
      const timedOutCount = (session.vendors_contacted?.length || 0) -
      ...
      if (positiveResponses.length === 0) {
        ...
        message += `Unfortunately, none of the ${
          session.vendors_contacted?.length || 0
        } pharmacies I contacted have all the items you're looking for.\n\n`;

        if (negativeResponses.length > 0) {
          message +=
            `${negativeResponses.length} pharmacy(ies) confirmed they don't have the items.\n`;
        }
        ...
        message =
          `✅ *Good news!* ${positiveResponses.length} pharmacy(ies) confirmed they have your items:\n\n`;
        ...
        positiveResponses.forEach((response, i) => {
          ...
          if (
            response.price_quoted &&
            Object.keys(response.price_quoted).length > 0
          ) {
          ...
          if (response.availability_details) {
            ...
            message += `   📝 ${details}${
              response.availability_details.length > 100 ? "..." : ""
            }\n`;
          }
        ...
        if (negativeResponses.length > 0 || timedOutCount > 0) {
          ...
          if (negativeResponses.length > 0) {
            message +=
              `${negativeResponses.length} pharmacy(ies) don't have the items`;
          }
          if (timedOutCount > 0) {
        ...
        message +=
          `📞 _Reply with a number (1-${positiveResponses.length}) to get contact details and directions!_\n`;
