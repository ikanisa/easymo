...
import { assertSpyCalls, spy } from "https://deno.land/std@0.168.0/testing/mock.ts";
...
Deno.test("confirmPurchase - success flow", async () => {
  ...
  const mockSupabase = {
    ...
    rpc: (func: string, args: any) => Promise.resolve({ data: null, error: null })
  };

  const rpcSpy = spy(mockSupabase, "rpc");
  const notifierSpy = spy(async (to: string, body: string) => {});

  const result = await confirmPurchase(mockSupabase, "p123", "momo-tx-123", notifierSpy);
