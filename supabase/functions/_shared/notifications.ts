...
const WHATSAPP_API_BASE_URL = Deno.env.get('WHATSAPP_API_BASE_URL') ?? 'https://graph.facebook.com/v20.0';
...
export async function sendTemplateNotification(params: TemplateNotification) {
  ...
  try {
    ...
    if (response.ok) {
      await client
        .from('notifications')
        .update({ status: 'sent', payload: json, sent_at: new Date().toISOString() })
        .eq('id', inserted.id);
