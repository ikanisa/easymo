...
Deno.serve(async (req) => {
  ...
  const driversPromise = supabase.from("driver_status").select("count", {
    count: "exact",
  }).eq("online", true);
  const tripsPromise = supabase.from("trips").select("count", {
    count: "exact",
  }).eq("status", "open");
  const subsPromise = supabase.from("subscriptions").select("count", {
    count: "exact",
  ...
  if (driversRes.error || tripsRes.error || subsRes.error) {
    const msg = driversRes.error?.message ?? tripsRes.error?.message ??
      subsRes.error?.message ?? "unknown error";
    return new Response(
      JSON.stringify({
        error: msg,
      }),
