  ...
  try {
    ...
    if (action === 'drivers' && req.method === 'GET') {
      ...
      if (isNaN(lat) || isNaN(lng) || !vehicleType) {
        return new Response(JSON.stringify({ error: 'lat, lng and vehicle_type are required' }), { status: 400 });
      }
      const settings = await getSettings();
      const radiusKm = radiusParam ? Number(radiusParam) : settings?.search_radius_km ?? 5;
      ...
      const { data, error } = await supabase
        ...
        .limit(max);
      if (error) throw error;
      return new Response(JSON.stringify({ drivers: data ?? [] }), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }
    if (action === 'passengers' && req.method === 'GET') {
      ...
      const driverRefCode = url.searchParams.get('driver_ref_code') ?? undefined;
      if (!forceAccess) {
        return new Response(JSON.stringify({ access: false, reason: 'no_access' }), { status: 200, headers: { 'Content-Type': 'application/json' } });
      }
      if (isNaN(lat) || isNaN(lng) || !vehicleType) {
        return new Response(JSON.stringify({ error: 'lat, lng and vehicle_type are required' }), { status: 400 });
      }
      ...
      const { data, error } = await supabase
        ...
        .limit(max);
      if (error) throw error;
      return new Response(JSON.stringify({ access: true, trips: data ?? [] }), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }
    if (action === 'profile' && req.method === 'GET') {
      const ref = url.searchParams.get('ref');
      if (!ref) {
        return new Response(JSON.stringify({ error: 'ref is required' }), { status: 400 });
      }
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
    ...
    if (action === 'schedule_passenger' && req.method === 'POST') {
      ...
      if (isNaN(lat) || isNaN(lng) || !vehicleType || !refCode) {
        return new Response(JSON.stringify({ error: 'lat, lng, vehicle_type and ref_code are required' }), { status: 400 });
      }
      const { data, error } = await supabase
        .from('trips')
        .insert({
        ...
        .maybeSingle();
      if (error) throw error;
      return new Response(JSON.stringify({ trip: data }), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }
    if (action === 'schedule_driver' && req.method === 'POST') {
      ...
      const { data, error } = await supabase
        ...
        .maybeSingle();
      if (error) throw error;
      return new Response(JSON.stringify({ access: true, trip: data }), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }
