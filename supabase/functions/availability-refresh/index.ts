...
const CRON_ENABLED = (Deno.env.get("AVAILABILITY_REFRESH_CRON_ENABLED") ?? "true").toLowerCase() !== "false";
...
async function runRefresh(trigger: 'http' | 'cron') {
  ...
  if (parkingError || availabilityError) {
    ...
    return {
      ok: false,
      error: parkingError?.message ?? availabilityError?.message ?? 'query_failed',
    };
  }
