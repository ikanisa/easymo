...
DO $$ 
BEGIN 
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Managers can read their mappings' AND tablename = 'bar_restaurant_map') THEN 
    CREATE POLICY "Managers can read their mappings"
...
DO $$ 
BEGIN 
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Service role manage mappings' AND tablename = 'bar_restaurant_map') THEN 
    CREATE POLICY "Service role manage mappings"
    ON public.bar_restaurant_map
    FOR ALL
    TO service_role
    USING (true)
