...
-- Add comment and search function only if the base table exists
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Skipping property standardization: property_listings table missing.';
    RETURN;
  END IF;
  ...
  EXECUTE 'DROP FUNCTION IF EXISTS search_properties_unified';
  ...
  EXECUTE 'COMMENT ON FUNCTION search_properties_unified IS ''Unified property search with spatial and text search. Created during Real Estate consolidation.''';
END $$;
