...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'postgis') THEN
    -- Create geography indexes for proximity search
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_marketplace_listings_geo ON public.marketplace_listings USING GIST(
      ST_SetSRID(ST_MakePoint(lng, lat), 4326)::geography
    ) WHERE lat IS NOT NULL AND lng IS NOT NULL';
    
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_marketplace_buyer_intents_geo ON public.marketplace_buyer_intents USING GIST(
      ST_SetSRID(ST_MakePoint(lng, lat), 4326)::geography
    ) WHERE lat IS NOT NULL AND lng IS NOT NULL';
  END IF;
END
