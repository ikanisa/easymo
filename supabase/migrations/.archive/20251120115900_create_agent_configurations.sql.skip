-- Dual LLM Provider Infrastructure (Standalone Version)
-- Creates all necessary tables including agent_configurations
...
CREATE TABLE IF NOT EXISTS agent_configurations (
  ...
  -- NEW: LLM Provider columns
  primary_provider VARCHAR(20) DEFAULT 'openai' CHECK (primary_provider IN ('openai', 'gemini')),
  fallback_provider VARCHAR(20) CHECK (fallback_provider IN ('openai', 'gemini')),
...
COMMENT ON TABLE agent_configurations IS 'Configuration for AI agents with dual LLM provider support';
COMMENT ON COLUMN agent_configurations.primary_provider IS 'Primary LLM provider (openai or gemini)';
...
-- Create General Broker configuration
