-- Remove Uganda and Kenya from bars search country filtering
-- =========================================================================================
BEGIN;

-- Update nearby_bars_by_preference to only allow RW, Malta, TZ countries
...
AS $$
  ...
  WHERE
    ...
    -- Country filter: Only RW, Malta, TZ (removed UG and KE)
    AND (
      UPPER(b.country) IN ('RW', 'RWANDA', 'MALTA', 'MT', 'TANZANIA', 'TZ')
    )
...
COMMENT ON FUNCTION public.nearby_bars_by_preference IS 'Search for nearby bars & restaurants from business table (RW, Malta, TZ only), filtered by preference';

SELECT 'Bars search updated: removed Uganda and Kenya' AS status;
