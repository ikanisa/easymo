-- ============================================================================
-- Final Fix for ensure_whatsapp_user Function
-- Migration: 20251216150000_fix_ensure_whatsapp_user_final.sql
-- Date: 2025-12-16
--
-- PURPOSE: 
-- Force recreate the function with correct syntax to fix ambiguous column reference
-- This migration uses DROP FUNCTION IF EXISTS CASCADE to ensure clean recreation
-- ============================================================================

BEGIN;

-- Drop the function completely to ensure clean recreation
DROP FUNCTION IF EXISTS public.ensure_whatsapp_user(TEXT, TEXT) CASCADE;

-- Recreate with correct syntax using table alias
...
BEGIN
  ...
  -- Warn if phone number seems incomplete
  ...
  WHERE p.wa_id = v_digits OR p.wa_id = v_normalized_phone
  LIMIT 1;
  ...
  IF NOT FOUND THEN
    ...
    INTO v_existing_profile
    FROM public.profiles p
    WHERE p.phone_number = v_digits OR p.phone_number = v_normalized_phone
    LIMIT 1;
  END IF;
  ...
  IF FOUND THEN
    -- Use table alias 'p' in INSERT statement to avoid ambiguity
  ...
  -- STEP 3: No existing user or profile found - return NULL
  RETURN;
