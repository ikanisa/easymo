...
-- Replace function to enforce a simple daily cap per sender (1000 TOK/day)
...
BEGIN
  ...
  -- Daily cap check (UTC day)
  SELECT COALESCE(SUM(amount_minor), 0) INTO v_daily_sent
  FROM public.wallet_transactions
  WHERE profile_id = p_sender
    AND currency = 'TOK'
    AND direction = 'debit'
    AND description = 'Token transfer'
    AND occurred_at >= date_trunc('day', timezone('utc', now()))
    AND occurred_at < date_trunc('day', timezone('utc', now())) + interval '1 day';

  IF v_daily_sent + p_amount > v_daily_cap THEN
    reason := 'daily_cap_exceeded'; RETURN NEXT; RETURN;
  END IF;
