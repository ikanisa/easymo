...
const DATABASE_URL = "postgresql://postgres:Pq0jyevTlfoa376P@db.lhbowpbcpwoiparwnwgt.supabase.co:5432/postgres";

const csvData = `bar name,bar_id,item name,price,category
...
async function uploadMenuItems() {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      const [barName, barId, itemName, priceStr, category] = parts;
      const price = parseFloat(priceStr);
      
      if (!barId || !itemName || isNaN(price)) {
        console.warn('Skipping invalid data:', { barName, barId, itemName, price });
        skipped++;
        continue;
      }
      ...
      try {
        await client.query(`
          INSERT INTO bar_menu_items (bar_id, bar_name, item_name, price, category, is_available)
          VALUES ($1, $2, $3, $4, $5, true)
          ON CONFLICT (bar_id, item_name, category) DO UPDATE
          SET price = EXCLUDED.price,
              bar_name = EXCLUDED.bar_name,
              updated_at = NOW()
        `, [barId, barName, itemName, price, category]);
