...
export class RecommendationEngine {
  ...
  async loadUserPreferences(userId: string): Promise<void> {
    const { data: profile } = await this.supabase
      .from('user_profiles')
      .select('dietary_restrictions, favorite_categories, price_preference')
      .eq('id', userId)
      .single();
    ...
    const previousItems = orders?.flatMap(o => 
      (o.items as any[])?.map((i: any) => i.menu_item_id) || []
    ) || [];
  ...
  private calculateItemScore(item: MenuItem): number {
    ...
    if (this.context.timeOfDay === 'morning' && item.category === 'breakfast') {
      score += 40;
    }
    if (this.context.timeOfDay === 'afternoon' && item.category === 'lunch') {
      score += 30;
    }
    if (this.context.timeOfDay === 'evening' && item.category === 'dinner') {
      score += 35;
    }
    if (this.context.timeOfDay === 'night' && item.category === 'drinks') {
      score += 40;
    }
