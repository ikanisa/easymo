...
export function validateEnv<T extends z.ZodRawShape>(
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      logger.error(
        { issues, environment: process.env.NODE_ENV },
...
export const commonEnvSchemas = {
  ...
  /** Boolean flag */
  booleanFlag: (defaultValue = false) =>
    z
      .string()
      .optional()
...
export function createServiceEnvSchema<T extends z.ZodRawShape>(
  ...
  return z.object({
    ...baseSchema,
    ...customSchema,
  }) as z.ZodObject<T & z.ZodRawShape>;
}

/**
 * Mask sensitive environment variables for logging
 */
export function maskSensitiveEnv(env: Record<string, any>): Record<string, string> {
  const sensitiveKeys = [
    'PASSWORD',
    'SECRET',
    'KEY',
