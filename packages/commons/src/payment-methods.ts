...
export const PAYMENT_METHOD_REGIONS: Record<PaymentMethod, string[]> = {
...
export function isValidPaymentMethod(method: string): method is PaymentMethod {
  return Object.values(SUPPORTED_PAYMENT_METHODS).includes(method as PaymentMethod);
}

/**
 * Get payment method for a region
 */
export function getPaymentMethodForRegion(region: string): PaymentMethod | null {
  ...
  if (
    normalizedRegion.includes('malta') ||
 ...
 */
export function validatePaymentMethod(method: string): asserts method is PaymentMethod {
