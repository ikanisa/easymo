...
export async function POST(request: Request) {
  return withRouteInstrumentation('driver.availability.update.POST', request, async ({ logger, traceId }) => {
    ...
    try {
      ...
      logger.error({ event: 'driver.availability.update.invalid_json', err: error });
    ...
    if (fetchError) {
      logger.error({ event: 'driver.availability.update.fetch_failed', err: fetchError });
    ...
    if (error) {
      logger.error({ event: 'driver.availability.update_failed', err: error });
