...
export async function POST(request: Request) {
  const auth = requireAuth(request, { requireRole: 'admin' });
  ...
  try {
    ...
    console.error('broker.candidates.invalid_json', error);
  ...
  if (parkingError) {
    console.error('broker.candidates.parking_failed', parkingError);
  ...
  if (liveError) {
    console.error('broker.candidates.live_failed', liveError);
  ...
  if (Array.isArray(liveData)) {
    for (const row of liveData as any[]) {
      ...
      const dropoffDistance = row.dropoff_distance_km != null
  ...
  console.warn('telemetry.broker_shortlist_generated', {
  ...
  return jsonOk({ request_id: requestId, candidates });
}
