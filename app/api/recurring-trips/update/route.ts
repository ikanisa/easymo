...
export async function POST(request: Request) {
  return withRouteInstrumentation('recurring-trips.update.POST', request, async ({ logger, traceId }) => {
    ...
    try {
      ...
      logger.error({ event: 'recurring_trips.update.invalid_json', err: error });
    ...
    if (fetchError) {
      logger.error({ event: 'recurring_trips.update.fetch_failed', err: fetchError });
    ...
    if (favoriteIds.length > 0) {
      ...
      if (favoriteError) {
        logger.error({ event: 'recurring_trips.update.favorite_lookup_failed', err: favoriteError });
    ...
    if (error) {
      logger.error({ event: 'recurring_trips.update_failed', err: error });
