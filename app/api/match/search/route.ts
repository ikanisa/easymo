...
export async function POST(request: Request) {
  ...
  if (favoriteIds.length > 0) {
    ...
    if (favoriteError) {
      console.error('match.search.favorite_lookup_failed', favoriteError);
  ...
  if (
    ...
    if (parkingError) {
      console.error('match.search.parking_failed', parkingError);
  ...
  if (originFavorite || destinationFavorite) {
    console.warn('telemetry.favorite_used_for_match', {
      user_id: auth?.userId,
  ...
  if (parkingCandidates.length > 0) {
    console.warn('telemetry.match_from_favorite', {
