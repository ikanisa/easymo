  ...
  const handleSignedUrl = useCallback(async (object: StorageObject) => {
    try {
      const response = await fetch(
        `/api/files/signed-url?bucket=${encodeURIComponent(object.bucket)}&path=${encodeURIComponent(object.path)}`
      );
      ...
      if (data?.integration?.status === 'degraded') {
        pushToast(data.integration.message ?? 'Signed URL generated in degraded mode.', 'info');
      }
    } catch (error) {
