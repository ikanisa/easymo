...
export function NotificationsTable({ initialData }: NotificationsTableProps) {
  const { data, error, mutate } = useSWR<{ data: NotificationOutbox[]; integration?: { target: string; status: 'ok' | 'degraded'; reason?: string; message?: string } }>(
  ...
  const columns = useMemo<ColumnDef<NotificationOutbox>[]>(
    () => [
      ...
      },
      {
        header: 'Sent',
        accessorKey: 'sentAt',
        cell: ({ row }) => (row.original.sentAt ? new Date(row.original.sentAt).toLocaleString() : '—')
      },
      {
        header: 'Actions',
        id: 'actions',
        cell: ({ row }) => (
          <div className={styles.actions}>
            ...
            <button
              type="button"
              onClick={() => setPendingAction({ id: row.original.id, action: 'cancel' })}
              disabled={isProcessing}
    ...
    <div className="stack">
      ...
      <DataTable
        ...
        globalFilterFn={(row, value) =>
          `${row.type} ${row.toRole}`.toLowerCase().includes(value.toLowerCase())
        }
        downloadFileName="notifications.csv"
      />
      {integration ? <IntegrationStatusBadge integration={integration} label="Notifications dispatcher" /> : null}
