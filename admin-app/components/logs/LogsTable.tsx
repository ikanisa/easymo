...
export function LogsTable({ data }: LogsTableProps) {
  ...
  const filteredData = useMemo(() => {
    return data.filter((event) => {
      ...
      const targetMatch = filters.target ? event.targetTable === filters.target : true;
    ...
    <div className="stack">
      <div className="filters">
        ...
        <label>
          ...
          <input
            ...
            onChange={(event) => setFilters((prev) => ({ ...prev, target: event.target.value || undefined }))}
      ...
      <DataTable
        ...
        globalFilterFn={(row, value) =>
          `${row.actor} ${row.action} ${row.targetTable}`.toLowerCase().includes(value.toLowerCase())
        }
