...
export function LogsClient() {
  ...
  if (isLoading) {
    return <LoadingState title="Loading logs" description="Fetching latest audit and voucher events." />;
  }
        ...
        {filteredAudit.length ? (
          <ul className="mt-6 space-y-3 text-sm">
            {filteredAudit.map((entry) => (
              ...
              >
                <strong className="text-[color:var(--color-foreground)]">{entry.action}</strong>{' '}
        ...
        {data.events.length ? (
          <ul className="space-y-2 text-sm">
            {data.events.map((event) => (
              ...
              >
                <strong className="text-[color:var(--color-foreground)]">{event.orderId}</strong> – {event.type} –{' '}
