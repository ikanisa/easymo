...
export function DataTable<TData>({
  ...
  const handleDownload = () => {
    const csv = Papa.unparse(table.getCoreRowModel().rows.map((row) => row.original as Record<string, unknown>));
  ...
  return (
    <div className={styles.wrapper} style={wrapperStyle}>
      <div className={styles.toolbar}>
        ...
        <button type="button" className={styles.exportButton} onClick={handleDownload}>
          Export CSV
        </button>
      </div>
      ...
      <div className={styles.tableContainer} role="table" aria-busy={isLoading}>
        ...
        <div role="rowgroup" className={styles.body} ref={containerRef}>
          {isLoading ? (
            <div role="row" className={classNames(styles.row, styles.placeholderRow)}>
