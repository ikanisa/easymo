...
export function VoucherGenerationForm() {
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit}>
      <div className={styles.row}>
        <label className={styles.multilineField}>
          ...
          <textarea
            ...
            rows={4}
          />
          <small className={styles.helper}>Paste one MSISDN per line or separate with commas.</small>
        </label>
        <label>
          <span>Amount</span>
          <input
      ...
      {integration ? <IntegrationStatusBadge integration={integration} label="Voucher issuance" /> : null}
          ...
          <ul>
            {results.map((entry) => (
              <li key={entry.voucherId}>
                <strong>{entry.voucherId}</strong> → {entry.msisdn} ({entry.status})
              </li>
            ))}
          </ul>
        </div>
