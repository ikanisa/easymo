...
export function VoucherPreviewButton({ voucher }: VoucherPreviewButtonProps) {
  ...
  const [preview, setPreview] = useState<{ imageUrl?: string | null; pdfUrl?: string | null; expiresAt?: string | null } | null>(null);
  ...
  const openPreview = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPreview({ imageUrl: data.imageUrl ?? null, pdfUrl: data.pdfUrl ?? null, expiresAt: data.expiresAt ?? null });
        if (data.status === 'degraded' || data.status === 'not_configured') {
          setStatusMessage(data.message ?? 'Voucher preview running in degraded mode. Showing design mock.');
        ...
        setStatusMessage('Unable to reach preview service. Showing design mock instead.');
      }
    } catch (error) {
    ...
    <>
      ...
      {isOpen ? (
        <Modal title="Voucher preview" onClose={() => setIsOpen(false)}>
          {integration ? <IntegrationStatusBadge integration={integration} label="Preview bridge" /> : null}
          ...
          />
          {preview?.imageUrl ? (
            <a className={styles.downloadLink} href={preview.imageUrl} target="_blank" rel="noreferrer">
              Download preview image
            </a>
          ) : null}
          {preview?.pdfUrl ? (
            <a className={styles.downloadLink} href={preview.pdfUrl} target="_blank" rel="noreferrer">
