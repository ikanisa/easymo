...
export function CampaignWizardMock({ templates }: CampaignWizardMockProps) {
  ...
  const [campaignType, setCampaignType] = useState<"promo" | "voucher">(
    "voucher",
  );
  ...
  const canSubmit = Boolean(
    selectedTemplate && campaignName.trim() && csvPreview && csvPreview > 0,
  );

  const handleCreateDraft = async () => {
    if (!canSubmit) {
      pushToast(
        "Select a template, provide a name, and upload recipient CSV first.",
        "error",
      );
      return;
    }
    ...
    <div className={styles.wrapper}>
        ...
        {selectedTemplate
          ...
          : (
            <p className={styles.helper}>
              Select a template to map campaign variables.
            </p>
          )}
      </section>
      ...
      <section>
        ...
        {csvPreview !== null
          ? (
            <p className={styles.helper}>
              {csvPreview} rows parsed successfully.
            </p>
          )
          : null}
      </section>
      ...
      <section>
        ...
        <div className={styles.formGrid}>
          ...
          <label>
            ...
            <select
              value={campaignType}
              onChange={(event) =>
                setCampaignType(event.target.value as "promo" | "voucher")}
            >
        ...
        {integration
          ...
          : null}
        {resultMessage
          ? <p className={styles.helper}>{resultMessage}</p>
          : null}
        {!canSubmit
          ? (
            <p className={styles.helperMuted}>
              Select a template, provide a name, and upload recipients before
              saving.
            </p>
          )
          : null}
