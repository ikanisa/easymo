...
export function OrdersTable({ data, onSelectOrder }: OrdersTableProps) {
  const [filters, setFilters] = useState<{ status?: string; bar?: string; stuck?: boolean }>({});
  ...
  const columns = useMemo<ColumnDef<Order>[]>(() => {
    ...
    return [
      ...baseColumns,
      {
        ...
        id: 'actions',
        cell: ({ row }) => (
          <button type="button" className="link-muted" onClick={() => onSelectOrder(row.original)}>
            Override
          </button>
        )
      }
  ...
  const filteredData = useMemo(() => {
    return data.filter((order) => {
      ...
      const stuckMatch = filters.stuck
        ? Date.now() - new Date(order.createdAt).getTime() > 15 * 60 * 1000 && order.status === 'pending'
        : true;
    ...
    <div className="stack">
      <div className="filters">
        ...
        <label>
          ...
          <input
            value={filters.bar ?? ''}
            onChange={(event) => setFilters((prev) => ({ ...prev, bar: event.target.value || undefined }))}
        ...
        <label className="filters__field">
          ...
          <select
            ...
            onChange={(event) =>
              setFilters((prev) => ({ ...prev, stuck: event.target.value === 'yes' ? true : undefined }))
            }
          >
      ...
      <DataTable
        ...
        globalFilterFn={(row, value) =>
          `${row.id} ${row.barName} ${row.table ?? ''}`.toLowerCase().includes(value.toLowerCase())
        }
