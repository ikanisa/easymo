...
export const mockBars: Bar[] = [
  {
    ...
    paymentInstructions: 'Dial the MoMo code and confirm table number on ticket.'
  },
  {
    id: 'bar-2',
...
export const mockVouchers: Voucher[] = Array.from({ length: 25 }, (_, index) => {
  const statusCycle: Voucher['status'][] = ['issued', 'sent', 'redeemed', 'expired', 'void'];
...
export const mockTimeseries: TimeseriesPoint[] = Array.from({ length: 14 }, (_, index) => {
...
export const mockMenuVersions: MenuVersion[] = mockBars.flatMap((bar, index) => [
  {
...
export const mockOcrJobs: OcrJob[] = Array.from({ length: 6 }, (_, idx) => {
  const statusCycle: OcrJob['status'][] = ['queued', 'processing', 'success', 'error'];
  return {
    id: `ocr-${idx + 1}`,
...
export const mockOrders: Order[] = Array.from({ length: 18 }, (_, index) => {
  ...
  return {
    ...
    status: ['pending', 'confirmed', 'ready', 'completed', 'cancelled'][index % 5],
...
export const mockOrderEvents: OrderEvent[] = mockOrders.slice(0, 10).map((order, index) => ({
  id: `event-${order.id}`,
  orderId: order.id,
  type: ['created', 'vendor_ack', 'preparing', 'completed', 'cancelled'][index % 5],
  status: order.status,
...
export const mockWebhookErrors: WebhookError[] = Array.from({ length: 6 }, (_, idx) => ({
  id: `webhook-${idx + 1}`,
...
export const mockNotifications: NotificationOutbox[] = Array.from({ length: 15 }, (_, idx) => ({
  id: `notif-${idx + 1}`,
...
export const mockAuditEvents: AuditEvent[] = Array.from({ length: 12 }, (_, idx) => ({
  id: `audit-${idx + 1}`,
