...
const barsKey = (params: BarsQueryParams) => ['bars', params] satisfies QueryKey;

export function fetchBars(params: BarsQueryParams = { limit: 100 }): Promise<PaginatedResult<Bar>> {
  return listBars(params);
}

export function useBarsQuery(
  params: BarsQueryParams = { limit: 100 },
  options?: UseQueryOptions<PaginatedResult<Bar>, unknown, PaginatedResult<Bar>>
) {
  return useQuery({
