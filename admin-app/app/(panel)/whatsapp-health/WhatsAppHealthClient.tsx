...
import { useNotificationsQuery, type NotificationsQueryParams } from '@/lib/queries/notifications';
...
export function WhatsAppHealthClient({ initialNotificationParams = { limit: 200 } }: WhatsAppHealthClientProps) {
  ...
  const notifications = useMemo(() => notificationsQuery.data?.data ?? [], [notificationsQuery.data?.data]);

  const { total, successCount, failureCount, queuedCount, successRate } = useMemo(() => {
    if (!notifications.length) {
      return { total: 0, successCount: 0, failureCount: 0, queuedCount: 0, successRate: '—' };
    }
    ...
    return { total, successCount: success, failureCount: failure, queuedCount: queued, successRate: rate };
  }, [notifications]);
    ...
    <div className="admin-page">
      ...
      <SectionCard title="Delivery overview" description="High-level stats from the notification outbox.">
        {notificationsQuery.isLoading ? (
          <LoadingState title="Loading notifications" description="Calculating delivery metrics." />
        ) : total ? (
          <div className="grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl border border-[color:var(--color-border)]/40 bg-[color:var(--color-surface)]/60 px-4 py-4">
              <strong className="text-sm text-[color:var(--color-muted)]">Success rate</strong>
              <p className="text-2xl font-semibold text-[color:var(--color-foreground)]">{successRate}%</p>
            </div>
            <div className="rounded-2xl border border-[color:var(--color-border)]/40 bg-[color:var(--color-surface)]/60 px-4 py-4">
              <strong className="text-sm text-[color:var(--color-muted)]">Queued</strong>
              <p className="text-2xl font-semibold text-[color:var(--color-foreground)]">{queuedCount}</p>
            </div>
            <div className="rounded-2xl border border-[color:var(--color-border)]/40 bg-[color:var(--color-surface)]/60 px-4 py-4">
              <strong className="text-sm text-[color:var(--color-muted)]">Failed</strong>
              <p className="text-2xl font-semibold text-[color:var(--color-foreground)]">{failureCount}</p>
            </div>
          </div>
        ) : (
          <EmptyState
      ...
      >
        {webhookErrorsQuery.isLoading ? (
          <LoadingState title="Loading webhook errors" description="Fetching latest webhook failures." />
        ) : (
