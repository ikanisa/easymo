...
export function NotificationsClient({ initialParams = { limit: 200 } }: NotificationsClientProps) {
    ...
    <div className="admin-page">
        ...
        actions={
          <div className="flex flex-wrap items-center gap-3">
            <label className="text-sm text-[color:var(--color-muted)]">
              ...
              <select
                value={params.status ?? ''}
                onChange={(event) => setParams((prev) => ({ ...prev, status: event.target.value || undefined }))}
      ...
      >
        {notificationsQuery.isLoading ? (
          <LoadingState title="Loading notifications" description="Fetching latest outbox entries." />
        ...
        {notifications.length ? (
          <ul className="grid gap-3 md:grid-cols-3">
            {STATUS_FILTERS.map((status) => (
              ...
              >
                <div className="text-xs uppercase tracking-[0.2em] text-[color:var(--color-muted)]">{status}</div>
