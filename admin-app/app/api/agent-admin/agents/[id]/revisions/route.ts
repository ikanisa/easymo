...
const createSchema = z.object({ instructions: z.string().min(1), tools: z.any().optional(), metadata: z.any().optional(), createdBy: z.string().optional() });
...
export async function GET(_request: Request, context: { params: { id: string } }) {
  try {
    ...
    const res = await fetch(`${base.replace(/\/$/, '')}/admin/agents/${id}/revisions`, { headers: headers() });
...
export async function POST(request: Request, context: { params: { id: string } }) {
  try {
    ...
    const base = getAgentCoreUrl();
    if (!base) return NextResponse.json({ error: 'agent_core_unavailable' }, { status: 503 });
    const res = await fetch(`${base.replace(/\/$/, '')}/admin/agents/${id}/revisions`, { method: 'POST', headers: headers(), body: JSON.stringify(payload) });
    const data = await res.json().catch(() => ({}));
    return NextResponse.json({ ok: res.ok, data }, { status: res.status });
  } catch (error) {
    return NextResponse.json({ error: 'bad_request', message: (error as Error).message }, { status: 400 });
