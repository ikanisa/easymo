...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    integration: {
      ...
      reason: "mock_preview",
      message:
        "Showing mock settings preview. Configure Supabase keys to load persisted values.",
    },
  });
}
...
export async function POST(request: Request) {
  try {
      ...
      const { error } = await adminClient
        ...
        .upsert([
          { key: "quiet_hours.rw", value: payload.quietHours },
          {
            ...
            value: { value: payload.throttlePerMinute },
          },
    ...
    const integration = supabaseStatus === "ok"
      ...
      : {
        ...
        reason: "mock_store",
        message:
          "Supabase not configured or update failed. Settings stored in memory only.",
      };
    ...
    return NextResponse.json({ error: "settings_update_failed" }, {
      status: 500,
    });
  }
}
