  ...
  try {
    ...
    const result = await withIdempotency(idempotencyKey, async () => {
      ...
      if (adminClient) {
        ...
        console.error(
          "Supabase voucher send update failed, using mock fallback",
          error,
        );
      }
      ...
      if (bridgeResult.ok) {
        ...
        console.error(
          "Voucher send bridge returned unexpected payload",
          parsed.error,
        );
        return {
          voucherId: payload.voucherId,
          status: "queued",
          message:
            "Voucher dispatched but response was invalid. Verify WhatsApp bridge payload.",
          integration: {
            target: "voucherSend",
            status: "degraded",
            reason: "http_error",
