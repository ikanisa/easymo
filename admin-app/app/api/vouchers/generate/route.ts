...
export async function POST(request: Request) {
  try {
    ...
    const result = await withIdempotency(idempotencyKey, async () => {
      ...
      if (bridgeResult.ok) {
        ...
        if (parsed.success) {
          ...
          return {
            ...
            stationScope: parsed.data.stationScope ?? payload.stationScope ??
              null,
            vouchers,
            message: parsed.data.message ??
              "Voucher generation successful via Edge Function.",
            integration: bridgeHealthy("voucherGenerate"),
          };
        }

        console.error(
          "Voucher generate bridge returned unexpected payload",
          parsed.error,
        );
      }
      ...
      const integrationNotice = bridgeResult.ok
        ? {
          ...
          message:
            "Voucher issuance bridge returned unexpected payload. Falling back to local generator.",
        }
      ...
      if (adminClient) {
        ...
        console.error(
          "Supabase voucher insert failed, falling back to mock",
          error,
        );
      }
      ...
      return {
        ...
        vouchers: fallback,
        message:
          "Mock voucher generation successful. No records were persisted.",
        integration: integrationNotice,
      };
    });
    ...
    return NextResponse.json({ error: "voucher_generate_failed" }, {
      status: 500,
    });
  }
}
