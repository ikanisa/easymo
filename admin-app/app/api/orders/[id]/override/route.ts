...
export async function POST(request: Request, context: { params: { id: string } }) {
  try {
    ...
    if (adminClient) {
      if (payload.action === 'cancel') {
        const { error } = await adminClient
          .from('orders')
          .update({ status: 'cancelled', cancelled_reason: payload.reason ?? 'Admin cancelled' })
          .eq('id', id);
        if (error) {
      ...
      if (payload.action === 'reopen') {
        const { error } = await adminClient.from('orders').update({ status: 'pending' }).eq('id', id);
        if (error) {
    ...
    if (!message) {
      message = payload.action === 'nudge' ? 'Vendor nudged (mock).' : `Order ${payload.action}ed (mock).`;
      if (payload.action === 'nudge') {
        integration = { target: 'orderOverride', status: 'degraded', reason: 'mock_store', message: 'Nudge stored for manual follow-up.' };
      }
    }
    ...
    return NextResponse.json({ orderId: id, status: payload.action, message, integration }, { status: 200 });
  } catch (error) {
    ...
    return NextResponse.json({ error: 'order_override_failed' }, { status: 500 });
  }
}
