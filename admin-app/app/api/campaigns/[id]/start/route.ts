  ...
  try {
    ...
    await recordAudit({
      ...
      summary: bridgeResult.ok
        ? "Campaign start dispatched"
        : "Campaign start requested (degraded)",
    });
    ...
    if (bridgeResult.ok) {
      ...
      console.error(
        "Campaign start bridge returned unexpected payload",
        parsed.error,
      );
      return NextResponse.json(
        {
          ...
          state: "running",
          message:
            "Campaign start dispatched but response was invalid. Verify dispatcher payload.",
          integration: {
    ...
    return NextResponse.json({ error: "campaign_start_failed" }, {
      status: 500,
    });
  }
}
