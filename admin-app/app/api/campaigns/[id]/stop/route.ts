  ...
  try {
    ...
    await recordAudit({
      ...
      summary: bridgeResult.ok
        ? "Campaign stop dispatched"
        : "Campaign stop requested (degraded)",
    });
    ...
    if (bridgeResult.ok) {
      ...
      console.error(
        "Campaign stop bridge returned unexpected payload",
        parsed.error,
      );
      return NextResponse.json(
        {
    ...
    return NextResponse.json({ error: "campaign_stop_failed" }, {
      status: 500,
    });
  }
}
