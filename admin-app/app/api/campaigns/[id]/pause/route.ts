...
export async function POST(_request: Request, context: { params: { id: string } }) {
  try {
    ...
    if (adminClient) {
      const { error } = await adminClient.from('campaigns').update({ status: 'paused' }).eq('id', id);
      if (error) {
    ...
    await recordAudit({
      ...
      targetId: id,
      summary: bridgeResult.ok ? 'Campaign pause dispatched' : 'Campaign pause requested (degraded)'
    });
    ...
    if (bridgeResult.ok) {
      ...
      console.error('Campaign pause bridge returned unexpected payload', parsed.error);
      return NextResponse.json(
        {
          id,
          state: 'paused',
    ...
    return NextResponse.json({ error: 'campaign_pause_failed' }, { status: 500 });
  }
}
