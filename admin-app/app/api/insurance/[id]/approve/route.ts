...
export async function POST(request: Request, context: { params: { id: string } }) {
  try {
    ...
    const bridgeResult = await callBridge(
      'insuranceWorkflow',
      { action: 'approve', quoteId: id, reviewerId: payload.reviewerId ?? null }
    );
    ...
    await recordAudit({
      ...
      targetId: id,
      summary: bridgeResult.ok ? 'Quote approval dispatched' : 'Quote approved (degraded)'
    });
    ...
    if (bridgeResult.ok) {
      ...
      console.error('Insurance approve bridge returned unexpected payload', parsed.error);
      return NextResponse.json(
        {
          quoteId: id,
          status: 'approved',
    ...
    return NextResponse.json({ error: 'insurance_approve_failed' }, { status: 500 });
  }
}
