...
These patterns provide the foundation for integrating hosted and bespoke tools into Easymo agents while keeping deployments compatible with Netlify hosting and the broader platform architecture.
...
| Deployment preview | Exercise agent flows on the Netlify preview URL | Confirm that hosted tools are reachable from Netlify edge and that environment variables are wired correctly. |

## 7. Deployment checklist for Netlify rollouts

1. Confirm that the agent environment variables for tool authentication are stored in Netlify project settings (Production & Preview).
