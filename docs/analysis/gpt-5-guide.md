...
1. Verify production builds use GPT-5-compatible environment variables and SDK versions.
...
- Store the GPT-5 API key in the hosting secret store (`OPENAI_API_KEY`) and never commit it locally.
...
Extend the earlier deployment checklist with hosting-specific integration steps:
...
3. **Prompt cache**: Enable Supabase Config Sync (or your platform's config store) to store prompt templates so hotfixes avoid redeployments.
4. **Monitoring**: Wire the Grafana/Tempo stack documented in `dashboards/` to receive GPT-5 latency + error metrics from the
   nginx ingress logs.
5. **Secrets rotation**: Schedule monthly key rotation through `scripts/rotate-openai-key.ts` to maintain compliance.
6. **Smoke tests**: Run `pnpm test:e2e --filter gpt5` before every production promotion; tests seed staging data.
