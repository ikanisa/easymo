...
## Query-Time Retrieval API (Next.js / Self-Hosted)

When deploying to self-hosted infrastructure, the API route should be stateless and edge-friendly. The following route performs a hybrid metadata + vector lookup and returns highlighted excerpts for display in a React client:
...
This pattern keeps the API route lightweight and ingress-friendly while letting the database enforce authorization.
...
These steps align with our deployment checklist for self-hosted services and help guarantee smooth reviews and rollouts.
...
* Admin API route `admin-app/app/api/agents/[id]/search/route.ts` exposes a semantic search endpoint tailored for the self-hosted deployment.
