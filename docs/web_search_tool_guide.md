...
- For production deployments (e.g., Netlify), ensure environment variables containing API keys allow the Responses API to access the `web_search` tool.
...
Teams deploying Easymo's front-end on Netlify should keep the following best practices in mind when shipping web search support:

- **Environment variables:** Define `OPENAI_API_KEY`, `EASYMO_WEB_SEARCH_ENABLED`, and any domain allow-lists in the Netlify Project Settings. Mark secrets as `Encrypted` and re-trigger builds after changes.
- **Edge vs. serverless functions:** Web search calls can increase cold-start times. For latency-sensitive experiences, pin the chat API routes to Netlify Edge Functions and pre-warm them through synthetic traffic.
- **Incremental rollouts:** Use Netlify environments (`Preview`, `Production`) to stage updates. Validate citation rendering in `Preview` before promoting.
- **Monitoring hooks:** Configure Netlify's built-in analytics and connect them with Easymo's observability stack (e.g., send metrics to DataDog or Grafana) so search-related regressions surface quickly.
- **Incident response:** Update the Netlify rollback playbook (`ROLLOUT_PLAN.md`) with search-specific mitigations, including toggling off the feature flag if web search outages occur.
