...
Ensure the following variables are configured locally before running Supabase, staging, or
production builds. The `scripts/check-env.mjs` helper validates this list.
...
Before deploying schema changes, run the policy audit to confirm row-level security is enabled on
all critical tables:
...
The script now validates that the agent orchestration tables (`agent_sessions`, `agent_quotes`, and
audit logs) include both RLS enablement and explicit policies in the compiled schema.

## Monitoring

Realtime channel telemetry and SLA breaches are collected through `/api/monitoring/realtime` and
`/api/monitoring/sla`. Ensure the webhook destinations above accept JSON payloads and respond with
`2xx` to avoid retry storms.
