...
The issue was a mismatch between what the TypeScript code expected (`address`, `distance`) and what
the database function returned (`location_text`, `distance_km`). Fixed by:
...
Users can now successfully view and select from the list of nearby bars after sharing their
location.
