...
The function `get_cached_location` already exists in production with a different return type signature.

## Root Cause
PostgreSQL doesn't allow `CREATE OR REPLACE FUNCTION` to change the return type of an existing function. The existing function must be dropped first.
...
## What This Means
The existing `get_cached_location` function in production has a DIFFERENT signature than our new one. This confirms that:
...
## Status
✅ Migration file updated with DROP statements
✅ Ready to deploy
