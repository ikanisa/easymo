...
**Issue: SMS Not Matching** → Check phone hash matches: `encode(sha256('781234567'::bytea), 'hex')`

**Issue: Webhook Not Routing** → Verify registration in `momo_webhook_endpoints` table

**Issue: Function Errors** → Check logs: `supabase functions logs momo-sms-webhook --tail`

**Issue: Portal Not Loading** → Check API routes: `curl http://localhost:3003/api/health`
