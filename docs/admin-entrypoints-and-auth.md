...
- **admin-app (Next.js admin panel)**: Uses the App Router. Public access is limited to `/` and `/login`, while middleware enforces sessions on all other routes via the `admin_session` cookie and returns 401 with cookie clearing when missing. The middleware also injects `x-request-id` headers for tracing on every request.
- **Login flow**: `/login` renders the `LoginForm` client component, which posts to `/api/auth/login`. The login API validates payloads with Zod, rate-limits by email, validates (but does not block on) CSRF tokens, verifies credentials, and writes a signed, HttpOnly `admin_session` cookie with configurable TTL before redirecting to `/dashboard`.
...
- **Waiter PWA**: The customer-facing PWA bootstraps Supabase via `lib/supabase.ts`, which builds a browser client from public env vars. The localized home page checks for an existing Supabase session, signs in anonymously when starting a session, and routes diners into the chat flow.
