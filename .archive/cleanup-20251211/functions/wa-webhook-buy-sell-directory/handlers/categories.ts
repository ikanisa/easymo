/**
 * Directory Categories Handler
 * 
 * Shows Buy & Sell categories as interactive list with pagination
 * Categories are fetched from buy_sell_categories table
 * Shows 9 categories per page with "Show More" option
 * 
 * State Keys:
 * - directory_menu_pagination: Category list pagination state
 */
...
/**
 * Show Buy & Sell categories to the user
 */
export async function showDirectoryCategories(
  userPhone: string,
  ...
  await logStructuredEvent("DIRECTORY_CATEGORIES_SENT", {
    page,
...
export async function handleShowMoreCategories(userPhone: string): Promise<void> {
  ...
  try {
    ...
    if (chatState.key !== "directory_menu_pagination" || !chatState.data) {
      await showDirectoryCategories(userPhone, "RW", 0);
      return;
    }
    ...
    await logStructuredEvent("DIRECTORY_SHOW_MORE_CATEGORIES", {
      userId: profile.user_id,
      page: nextPage,
    });

    // Show next page
    await showDirectoryCategories(userPhone, "RW", nextPage);

  } catch (error) {
    await logStructuredEvent("DIRECTORY_SHOW_MORE_CATEGORIES_ERROR", {
