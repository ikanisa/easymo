/**
 * WA-Agent-Support - Customer Support AI Agent
 * 
 * Standalone edge function for customer support.
 */

import { serve } from 'https://deno.land/std@0.168.0/http/server.ts';
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';
import { SupportAgent } from './core/support-agent.ts';
...
serve(async (req: Request): Promise<Response> => {
  ...
  const respond = (body: unknown, init: ResponseInit = {}): Response => {
    ...
    headers.set('X-Service', 'wa-agent-support');
    return new Response(JSON.stringify(body), { ...init, headers });
  };

  if (url.pathname.endsWith('/health')) {
    return respond({ status: 'healthy', service: 'wa-agent-support', agent: agent.type });
  }
  ...
  try {
    ...
    await logStructuredEvent('SUPPORT_AGENT_ERROR', { error: String(error), correlationId }, 'error');
